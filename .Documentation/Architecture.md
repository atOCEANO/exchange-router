...
**The budget is per process, and in local mode that process is yours.** All of the machinery below lives in the adapter, so it protects one Python process and nothing more. A service coordinates because every caller shares one set of adapters and therefore one budget. Two notebooks in local mode are two budgets, and the exchange sees the sum of both while neither can see the other. That is the operational difference between the modes, and where [Which mode to use](../README.md#which-mode-to-use) draws its line.
